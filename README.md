...
| [`tests/test-data/fake-china-cur/`](tests/test-data/fake-china-cur) | A ready-to-upload synthetic China CUR for August 2026: 7,580 rows across Ningxia and Beijing, in CNY |
| [`tests/test-data/sample-commercial-cur-2026-08.csv.gz`](tests/test-data/sample-commercial-cur-2026-08.csv.gz) | The synthetic commercial CUR the China sample was built from |
...
Everything in `tests/test-data/` is synthetic: made-up accounts, resources, tags and prices. The China sample covers EC2, EBS, S3, Lambda, a NAT gateway, cross-region transfer, a Compute Savings Plan, an RDS Reserved Instance, Enterprise Support fees and 6% VAT. At a rate of `0.13986014` USD per CNY, the relay should report 7,580 rows, CNY 10,717.04 and USD 1,498.89. Upload it with:

```bash
aws s3 sync ./tests/test-data/fake-china-cur/ s3://YOUR_SOURCE_BUCKET/
```

To rebuild it:

```bash
python3 tools/make_sample_commercial_cur.py --out tests/test-data/sample-commercial-cur-2026-08.csv.gz
python3 tools/make_fake_china_cur.py tests/test-data/sample-commercial-cur-2026-08.csv.gz \
  --out tests/test-data/fake-china-cur --prefix cur --report china-hourly --add-china-extras
```
